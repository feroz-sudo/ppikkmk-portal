...
import {
    ...
    ClipboardList, Lightbulb, Target, Flag, AlertTriangle, MessageSquare, BrainCircuit, Play, Save, Users,
    CheckCircle2
} from "lucide-react";
...
export default function KKClientFolderPage({ params }: PageProps) {
    const unwrappedParams = React.use(params);
    const { clientId } = unwrappedParams;
    const type = "KK"; // Hardcoded for this dedicated route
    ...
    // KK-relevant form manifest (Form 8 is in Programs & Events, not here)
    ...
    const handleSaveSummary = async () => {
        ...
        try {
            ...
            alert("Group Summary saved successfully.");
        } catch (error) {
            console.error(error);
    ...
    // Grouping sessions by Session Number
        ...
        <div className="max-w-5xl mx-auto pb-12">
            ...
            <div className="bg-upsi-navy rounded-xl shadow-md border-t-4 border-upsi-gold p-8 mb-6 text-white flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold flex items-center space-x-3 mb-2">
                        ...
                        <span>KK Group Session Folder</span>
                    </h1>
                    <p className="text-blue-100 flex items-center space-x-2">
                        <span>{client ? client.demographics.name : "Loading..."}</span>
                    </p>
                    <p className="text-xs text-blue-300 mt-2 pl-1">Kaunseling Kelompok (Group Counseling)</p>
                </div>
                <button
                    onClick={() => {
                        setShowNewSession(!showNewSession);
                        if (!showNewSession) {
                            // Auto-increment logic: find max sessionId and add 1
                            const maxSess = sessions.reduce((max, s) => {
                                const num = parseInt(s.sessionId);
                                return !isNaN(num) && num > max ? num : max;
                            }, 0);
                            setNewSessionId(String(maxSess + 1).padStart(2, '0'));
                        }
                    }}
                    className={`${sessions.length === 0 ? 'bg-green-600 hover:bg-green-700 animate-pulse' : 'bg-upsi-gold hover:bg-yellow-500'} text-white font-bold py-2 px-6 rounded-lg transition-all shadow-lg flex items-center space-x-2`}
                >
                    {sessions.length === 0 ? <CheckCircle2 size={20} /> : <PlusCircle size={20} />}
                    <span>{sessions.length === 0 ? "Start Session 01" : "Start New Session"}</span>
                </button>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-lg font-bold text-gray-800 flex items-center">
                        ...
                        Group Summary &amp; Overarching Goals
                    </h2>
                    <button
                ...
                <textarea
                    ...
                    placeholder="Document group therapeutic goals, critical breakthroughs, or long-term progress notes here. This persists across all sessions..."
                    value={summary}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-h-[400px]">
                <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    ...
                    <span className="text-xs text-gray-400 font-medium">{sortedGroupKeys.length} session(s) on record</span>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="p-16 text-center text-gray-500 flex flex-col items-center">
                        ...
                        <p className="mt-2 text-sm text-gray-400 max-w-sm">No group session documents have been logged yet. Use the &quot;Start New Session&quot; hub above to begin.</p>
                    </div>
                ) : (
                    <div className="p-6">
                        <div className="relative border-l-2 border-indigo-100 ml-4 space-y-8 pb-4">
                            {sortedGroupKeys.map((sessId) => {
                                ...
                                const sessDate = new Date(
                                    sessForms[0].createdAt instanceof Object && 'seconds' in sessForms[0].createdAt
                                        ? (sessForms[0].createdAt as { seconds: number }).seconds * 1000
                                        : sessForms[0].createdAt as Date
                                ).toLocaleDateString();
                                    ...
                                    <div key={sessId} className="relative pl-8">
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                {sessForms.map(f => {
                                                    const mappedForm = availableForms.find(af =>
                                                        af.name.replace(/\s+/g, '').toLowerCase().includes(f.formType.replace('Form', '').toLowerCase()) ||
                                                        f.formType === af.id
                                                    );
