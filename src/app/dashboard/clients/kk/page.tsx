    ...
    async function fetchClients() {
        ...
        try {
            ...
            for (const log of allLogs) {
                const kkMatch = (log.description || "").match(/PKKM\d+\/(\d{3})/i);
                if (kkMatch) {
                    const cId = kkMatch[1];
                    const key = `KK_${cId}`;
            ...
            const sortedKK = allClients.filter(c => c.type === "KK").sort((a, b) => a.clientId.localeCompare(b.clientId));
            
            for (const client of sortedKK) {
                const key = `KK_${client.clientId}`;
